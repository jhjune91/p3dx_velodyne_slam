...
int main(int argc, char **argv)
{
  ...
  double BASE_SPEED = -0.2, MOVE_TIME = 1.0, CLOCK_SPEED = 0.5;
  int count = 0;
  ...
  while(ros::ok() && count<MOVE_TIME/CLOCK_SPEED + 1)
      ...
      if (count == 0 || count == 1)
	{
      ...
      ROS_INFO_STREAM("The robot is now moving backward!");
      count++;
      ros::spinOnce();
      rate.sleep();
   }
  ...
  for (int i = 0; i < 2; i++)
    ...
    ROS_INFO_STREAM("The robot finished moving backward one second!");
