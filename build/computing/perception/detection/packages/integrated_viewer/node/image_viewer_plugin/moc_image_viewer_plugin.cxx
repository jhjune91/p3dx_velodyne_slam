/****************************************************************************
** Meta object code from reading C++ file 'image_viewer_plugin.h'
**
** Created by: The Qt Meta Object Compiler version 63 (Qt 4.8.6)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../../../../../../../../src/computing/perception/detection/packages/integrated_viewer/node/image_viewer_plugin/image_viewer_plugin.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'image_viewer_plugin.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 63
#error "This file was generated using the moc from 4.8.6. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_integrated_viewer__ImageViewerPlugin[] = {

 // content:
       6,       // revision
       0,       // classname
       0,    0, // classinfo
       3,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: signature, parameters, type, tag, flags
      44,   38,   37,   37, 0x08,
      85,   38,   37,   37, 0x08,
     125,   38,   37,   37, 0x08,

       0        // eod
};

static const char qt_meta_stringdata_integrated_viewer__ImageViewerPlugin[] = {
    "integrated_viewer::ImageViewerPlugin\0"
    "\0index\0on_image_topic_combo_box__activated(int)\0"
    "on_rect_topic_combo_box__activated(int)\0"
    "on_point_topic_combo_box__activated(int)\0"
};

void integrated_viewer::ImageViewerPlugin::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Q_ASSERT(staticMetaObject.cast(_o));
        ImageViewerPlugin *_t = static_cast<ImageViewerPlugin *>(_o);
        switch (_id) {
        case 0: _t->on_image_topic_combo_box__activated((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 1: _t->on_rect_topic_combo_box__activated((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 2: _t->on_point_topic_combo_box__activated((*reinterpret_cast< int(*)>(_a[1]))); break;
        default: ;
        }
    }
}

const QMetaObjectExtraData integrated_viewer::ImageViewerPlugin::staticMetaObjectExtraData = {
    0,  qt_static_metacall 
};

const QMetaObject integrated_viewer::ImageViewerPlugin::staticMetaObject = {
    { &rviz::Panel::staticMetaObject, qt_meta_stringdata_integrated_viewer__ImageViewerPlugin,
      qt_meta_data_integrated_viewer__ImageViewerPlugin, &staticMetaObjectExtraData }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &integrated_viewer::ImageViewerPlugin::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *integrated_viewer::ImageViewerPlugin::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *integrated_viewer::ImageViewerPlugin::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_integrated_viewer__ImageViewerPlugin))
        return static_cast<void*>(const_cast< ImageViewerPlugin*>(this));
    typedef rviz::Panel QMocSuperClass;
    return QMocSuperClass::qt_metacast(_clname);
}

int integrated_viewer::ImageViewerPlugin::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    typedef rviz::Panel QMocSuperClass;
    _id = QMocSuperClass::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 3)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 3;
    }
    return _id;
}
QT_END_MOC_NAMESPACE
